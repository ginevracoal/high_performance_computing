...
for MATRIXSIZE in 1024 2048 4096 8192; do
	...
	while [ $BLOCKSIZE -le $MATRIXSIZE ]; do
		...
		echo "$(perf stat -e  L1-dcache-load-misses ./faster_transpose.x $MATRIXSIZE $BLOCKSIZE 2>&1 | awk '/L1-dcache-load-misses/ {print($1)}')" | awk -F "." '{print $1 $2 $3}' >>cache_misses/cache_misses_$MATRIXSIZE.txt
