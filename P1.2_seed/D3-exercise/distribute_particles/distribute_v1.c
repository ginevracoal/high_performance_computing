...
double MAS(double d) {
  ...
  if (MAS_select == NGP) {
    ...
    if (distance < 0.5)
      result = (0.75 - pow(distance, 2));
    else if (distance >= 0.5 && distance < 1.5)
      result = (0.5 * pow(1.5 - distance, 2));
...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------
  // LOOP 1:  instead of calculating the distance, we consider the square
  // distance and check on $Rmax^2$, so we don't have to compute the square root
  // ---------------------------------------------------
