// instead of calculating the distance, we consider the square distance and
// check on $Rmax^2$, so we don't have to compute the square root
...
double MAS(double d) {
  ...
  if (MAS_select == NGP) {
    ...
    if (distance < 0.5)
      result = (0.75 - distance * distance);
    else if (distance >= 0.5 && distance < 1.5)
      result = (0.5 * (1.5 - distance) * (1.5 - distance));
...
int main(int argc, char **argv) {
  ...
  double *Grid;
  int Np, Ng, i, j, k, p;
  int register dx, dy, dz;
  double register Ng_inv;
  ...
  // for the sake of simplicity, we'll assume that all particles have the same
  // unitary mass

  // ---------------------------------------------------
  // LOOP 5:  uses the register keyword in order to suggest to the compiler that
  // the corresponding variables may need a faster access because they are
  // called several times, so it would be useful to keep them in the register.
  // ---------------------------------------------------
    ...
    for (i = 0; i < Ng; i++) {
      ...
      for (j = 0; j < Ng; j++) {
        ...
        for (k = 0; k < Ng; k++) {
          ...
          if (dist < RMax2) Grid[ijk(Ng, i, j, k)] += MAS(sqrt(dist));
        }
      }
    }
  }
