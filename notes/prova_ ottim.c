...
double Ng_inv = (double)1.0 / Ng; //perché devo specificare double????
...
double MAS(double d){
  double distance = fabs(d); //floating point absolute value
  double result;
...
int main(int argc, char **argv){
  ...
  //il numero totale di celle è Ng^3
  ...
  //step 1: tolgo la radice quadrata e trasformo Rmax in Rmax^2
  
  //step 2: avoid calls to pow()
  
  //step 3: avoid the floating point division defining Ng_inv
  
  //step 4-5: evito di fare un loop per ogni variabile, quindi faccio il calcolo dei prodotti tra le distanze solo una volta per ognuna
  
  //step 6: uso i registri
  
  printf(" v0 :: "); fflush(stdout);

  double register dist;
  double register half_size = 0.5 / Ng; // 1/Ng è la larghezza di ogni cella
  							   // half_size è il raggio
  ...
  // loop over all cells
  for(i = 0; i < Ng; i++){
    double register dx2 = x[p] - (double)i * Ng_inv + half_size; 
    dx2 = dx2*dx2;
